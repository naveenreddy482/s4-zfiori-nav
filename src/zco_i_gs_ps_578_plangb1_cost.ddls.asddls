...
@EndUserText.label: 'Gasline: GB Start on WBS Element'
...
define view entity ZCO_I_GS_PS_578_PLANGB1_COST
  ...
  inner join ZPS_I_BPJA as _BPJA on _BPJA.ObjectNumber = _psp.Objnr
{
  ...
  key _BPJA.FiscalYear,
      ...
      cast(sum(_BPJA.ValueYearTransCurrency) as bp_wgt preserving type ) as GBStart
}
where
      _BPJA.PlanBudgetVersion = 'GB1'
